...
export class ConversationStore {
    ...
    ensureUser(userId) {
        const now = Date.now();
        this.db
            .prepare(`INSERT OR IGNORE INTO users (id, created_at, updated_at, last_user_activity_at, last_agent_activity_at, metadata)
         VALUES (?, ?, ?, ?, ?, ?)`)
    ...
    ensureConversation(input) {
        ...
        const now = Date.now();
        this.db
            .prepare(`INSERT OR IGNORE INTO conversations (id, user_id, channel, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
            .run(input.conversationId, input.userId, input.channel, input.status || "active", now, now);
        this.db
            .prepare(`UPDATE conversations SET updated_at = ?, status = ? WHERE id = ?`)
    ...
    appendMessage(input) {
        ...
        this.db
          ...
          id, user_id, conversation_id, role, content, channel, created_at, metadata
            ...
            .run(id, input.userId, input.conversationId, input.role, input.content, input.channel, now, safeStringify(input.metadata || {}));
        this.db
            .prepare(`UPDATE conversations SET updated_at = ? WHERE id = ?`)
            .run(now, input.conversationId);
        if (input.role === "user") {
            this.db
                .prepare(`UPDATE users SET last_user_activity_at = ?, updated_at = ? WHERE id = ?`)
                .run(now, now, input.userId);
        }
        else if (input.role === "assistant") {
            this.db
                .prepare(`UPDATE users SET last_agent_activity_at = ?, updated_at = ? WHERE id = ?`)
                .run(now, now, input.userId);
        }
        return {
            id,
    ...
    getRecentMessages(input) {
        ...
        if (input.conversationId) {
           ...
           LIMIT ?`)
                .all(input.userId, input.conversationId, limit);
        }
        else {
            rows = this.db
                .prepare(`SELECT * FROM messages
...
export class KnowledgeStore {
    ...
    addMemoryItem(input) {
        ...
        this.db
          ...
          id, user_id, type, content, importance, confidence, status, created_at, updated_at, metadata
            ...
            .run(id, input.userId, input.type, input.content, clamp01(input.importance ?? 0.65), clamp01(input.confidence ?? 0.6), now, now, safeStringify(input.metadata || {}));
        this.addAuditLog({
            userId: input.userId,
    ...
    addEvidence(input) {
        ...
        this.db
          ...
          id, memory_item_id, message_id, user_id, confidence, excerpt, created_at, metadata
            ...
            .run(id, input.memoryItemId, input.messageId || null, input.userId, clamp01(input.confidence ?? 0.6), input.excerpt || null, now, safeStringify(input.metadata || {}));
        return id;
    }
    upsertUserTrait(input) {
        ...
        this.db
            .prepare(`INSERT INTO user_traits (
          id, user_id, trait_key, trait_value, confidence, contradiction_group,
          status, created_at, updated_at, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`)
            .run(id, input.userId, input.key, input.value, clamp01(input.confidence ?? 0.65), input.contradictionGroup || null, now, now, safeStringify(input.metadata || {}));
        this.addAuditLog({
            userId: input.userId,
    ...
    upsertAgentTrait(input) {
        ...
        this.db
            .prepare(`INSERT INTO agent_traits (
          id, trait_key, trait_value, confidence, status, created_at, updated_at, metadata
        ) VALUES (?, ?, ?, ?, 'active', ?, ?, ?)`)
            .run(id, input.key, input.value, clamp01(input.confidence ?? 0.8), now, now, safeStringify(input.metadata || {}));
        return id;
    }
    upsertRelationship(input) {
        ...
        this.db
            .prepare(`INSERT INTO relationships (
          id, user_id, subject, relation, object, confidence,
          status, created_at, updated_at, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`)
            .run(id, input.userId, input.subject, input.relation, input.object, clamp01(input.confidence ?? 0.6), now, now, safeStringify(input.metadata || {}));
        this.addAuditLog({
            userId: input.userId,
    ...
    getTopMemoryItems(userId, limit = 10) {
        const rows = this.db
    ...
    getUserTraits(userId, limit = 20) {
        const rows = this.db
            .prepare(`SELECT * FROM user_traits
         WHERE user_id = ? AND status = 'active'
        ...
        return rows.map((row) => ({
            ...
            confidence: Number(row.confidence || 0),
            contradictionGroup: typeof row.contradiction_group === "string"
                ? row.contradiction_group
                : undefined,
            createdAt: Number(row.created_at || 0),
        }));
    }
    getAgentTraits(limit = 20) {
        const rows = this.db
            .prepare(`SELECT * FROM agent_traits
         WHERE status = 'active'
         ORDER BY confidence DESC, created_at DESC
         LIMIT ?`)
    ...
    getRelationships(userId, limit = 20) {
        const rows = this.db
    ...
    getStatus(userId) {
        ...
        return {
            messages: asCount(`SELECT COUNT(*) AS count FROM messages WHERE user_id = ?`),
            memoryItems: asCount(`SELECT COUNT(*) AS count FROM memory_items WHERE user_id = ? AND status = 'active'`),
            userTraits: asCount(`SELECT COUNT(*) AS count FROM user_traits WHERE user_id = ? AND status = 'active'`),
            relationships: asCount(`SELECT COUNT(*) AS count FROM relationships WHERE user_id = ? AND status = 'active'`),
            learningJobsPending: asCount(`SELECT COUNT(*) AS count FROM learning_jobs WHERE user_id = ? AND status IN ('pending','failed')`),
            proactivePending: asCount(`SELECT COUNT(*) AS count FROM proactive_events WHERE user_id = ? AND status = 'pending'`),
        };
    }
    exportUserData(userId) {
        const selectAll = (table) => this.db
    ...
    addAuditLog(input) {
        ...
        this.db
            .prepare(`INSERT INTO memory_audit_log (
          id, user_id, action, scope, detail, created_at, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(id, input.userId, input.action, input.scope, input.detail, now, safeStringify(input.metadata || {}));
        return id;
    }
}
export class ContextAssembler {
    conversationStore;
    knowledgeStore;
    constructor(conversationStore, knowledgeStore) {
    ...
    buildContext(input) {
        ...
        const memoryItems = this.knowledgeStore.getTopMemoryItems(input.userId, input.memoryLimit || 16);
        const userTraits = this.knowledgeStore.getUserTraits(input.userId, input.traitLimit || 20);
        const agentTraits = this.knowledgeStore.getAgentTraits(16);
        const relationships = this.knowledgeStore.getRelationships(input.userId, 16);
        // Separate memory types for richer context
        const userMemories = memoryItems.filter((m) => m.type !== "self_reflection" &&
            m.type !== "system_audit" &&
        ...
        if (userTraits.length > 0) {
            ...
            if (grouped.size > 0) {
                sections.push("Things I've learned about them:", ...[...grouped.entries()].map(([k, v]) => `- ${k}: ${v}`));
            }
        }
        else {
            sections.push("I don't know much about this user yet — I should be curious and learn!");
        }
        // Section 2: Relationships
        if (relationships.length > 0) {
            sections.push("", "=== PEOPLE IN THEIR LIFE ===", ...relationships.map((r) => `- ${r.subject} ${r.relation} ${r.object}`));
        }
        // Section 3: Important memories (only user memories, skip reflections/curiosity/audit)
        if (userMemories.length > 0) {
            sections.push("", "=== THINGS I REMEMBER ===", ...userMemories.slice(0, 8).map((m) => `- ${m.content}`));
        }
...
export class LearningEngine {
    ...
    enqueueJob(input) {
        ...
        this.db
            .prepare(`INSERT INTO learning_jobs (
          id, user_id, conversation_id, type, payload, status,
          attempts, max_attempts, run_after, error, last_error_at,
          created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, 'pending', 0, ?, ?, NULL, NULL, ?, ?)`)
            .run(id, input.userId, input.conversationId, input.type, safeStringify(input.payload || {}), Math.max(1, Math.min(20, input.maxAttempts || 5)), input.runAfter || now, now, now);
        return id;
    }
    listPendingJobs(limit = 20, now = Date.now()) {
        const rows = this.db
            .prepare(`SELECT * FROM learning_jobs
         WHERE status IN ('pending', 'failed')
           AND run_after <= ?
         ORDER BY created_at ASC
         LIMIT ?`)
            .all(now, Math.max(1, Math.min(200, limit)));
        return rows.map((row) => ({
            id: String(row.id),
    ...
    markFailed(id, attempts, maxAttempts, error) {
        const now = Date.now();
        ...
        const retryDelayMs = Math.min(60 * 60 * 1000, 1000 * 2 ** Math.max(1, attempts));
        this.db
            .prepare(`UPDATE learning_jobs
...
export class AutonomyEngine {
    ...
    conversationStore;
    knowledgeStore;
    constructor(db, conversationStore, knowledgeStore) {
        this.db = db;
        this.conversationStore = conversationStore;
        this.knowledgeStore = knowledgeStore;
    }
    evaluateAndQueue(input) {
        ...
        const idleThresholdMs = Math.max(1, input.idleThresholdMs || DEFAULT_IDLE_THRESHOLD_MS);
        const lastActivity = this.conversationStore.getLastUserActivity(input.userId);
        const lastSentRow = this.db
            .prepare(`SELECT sent_at FROM proactive_events
         WHERE user_id = ? AND status = 'sent'
         ORDER BY sent_at DESC
         LIMIT 1`)
            .get(input.userId);
        const sentTodayRow = this.db
            .prepare(`SELECT COUNT(*) AS count FROM proactive_events
         WHERE user_id = ? AND status = 'sent' AND created_at >= ?`)
            .get(input.userId, startOfDayUtc(now));
        ...
        const createdEventIds = channels.map((channel) => this.queueProactiveEvent({
            userId: input.userId,
            channel,
            eventType: "check_in",
    ...
    queueProactiveEvent(input) {
        const now = Date.now();
        const id = `pro-${now}-${randomUUID()}`;
        this.db
            .prepare(`INSERT INTO proactive_events (
          id, user_id, channel, event_type, message, status, reason, created_at, sent_at, metadata
        ) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, NULL, ?)`)
            .run(id, input.userId, input.channel, input.eventType || "suggestion", input.message, input.reason || "manual_queue", now, safeStringify(input.metadata || {}));
        return id;
    }
    listPendingProactiveEvents(limit = 20) {
        const rows = this.db
            .prepare(`SELECT * FROM proactive_events
    ...
    markProactiveSent(id) {
        const now = Date.now();
        this.db
            .prepare(`UPDATE proactive_events
         SET status = 'sent', sent_at = ?, updated_at = ?
         WHERE id = ?`)
            .run(now, now, id);
    }
    markProactiveDropped(id, reason) {
        this.db
            .prepare(`UPDATE proactive_events
    ...
    createApprovalRequest(input) {
        ...
        this.db
          ...
          expires_at, created_at, updated_at, approved_at, rejected_at, metadata
            ...
            .run(id, input.userId, input.actionType, safeStringify(input.actionPayload), input.reason, token, expiresAt, now, now, safeStringify({ risk: "high" }));
        this.knowledgeStore.addAuditLog({
            userId: input.userId,
    ...
    rejectRequest(input) {
        ...
        this.db
         ...
         WHERE id = ?`)
            .run(now, String(input.reason || row.reason || "rejected"), now, input.requestId);
        this.knowledgeStore.addAuditLog({
            userId: String(row.user_id),
    ...
    rowToApprovalRequest(row) {
        return {
            ...
            expiresAt: Number(row.expires_at || 0),
            createdAt: Number(row.created_at || 0),
            approvedAt: typeof row.approved_at === "number"
                ? Number(row.approved_at)
                : undefined,
            rejectedAt: typeof row.rejected_at === "number"
...
export class MemoryV2 {
    ...
    constructor(db) {
        ...
        this.contextAssembler = new ContextAssembler(this.conversationStore, this.knowledgeStore);
        this.learningEngine = new LearningEngine(db);
        this.autonomyEngine = new AutonomyEngine(db, this.conversationStore, this.knowledgeStore);
    }
    ...
    async processPendingLearningJobs(input) {
        ...
        for (const job of jobs) {
            try {
                ...
                this.learningEngine.markFailed(job.id, attempts, job.maxAttempts, String(error?.message || error || "unknown learning error"));
            }
        }
...
function startOfDayUtc(timestampMs) {
    const d = new Date(timestampMs);
    return Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate());
}
function buildProactiveMessage(traits, memories, idleMs) {
    ...
    const intro = curiousIntros[Math.floor(Math.random() * curiousIntros.length)] ||
        curiousIntros[0];
    ...
    if (nonNameTraits.length > 0) {
        const trait = nonNameTraits[Math.floor(Math.random() * nonNameTraits.length)];
        if (trait) {
...
//# sourceMappingURL=memory-v2.js.map