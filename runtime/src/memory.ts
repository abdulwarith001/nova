...
export class MemoryStore {
  ...
  private loadProfiles(): void {
    ...
    for (const row of userRows) {
      try {
        ...
        if (row.key === "preferences") this.userProfile.preferences = value;
        else if (row.key === "goals") this.userProfile.goals = value;
        else if (row.key === "context") this.userProfile.context = value;
        else if (row.key === "name") this.userProfile.name = value;
        else if (row.key === "workStyle") this.userProfile.workStyle = value;
      } catch (e) {
    ...
    for (const row of agentRows) {
      try {
        const value = JSON.parse(row.value);
        if (row.key === "capabilities") this.agentProfile.capabilities = value;
  ...
  async search(
    ...
    const params: any[] = [escapedQuery, minImportance];
  ...
  async updateUserProfile(updates: Partial<UserProfile>): Promise<void> {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (value !== undefined) {
        ...
        (this.userProfile as any)[key] = value;
      }
    }
  }
  ...
  async updateAgentProfile(updates: Partial<AgentProfile>): Promise<void> {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (value !== undefined) {
        this.db
          .prepare(
            `INSERT OR REPLACE INTO agent_profile (key, value, updated_at) VALUES (?, ?, ?)`,
          )
          .run(key, JSON.stringify(value), timestamp);

        // Update in-memory profile
        (this.agentProfile as any)[key] = value;
      }
    }
  }
   ...
   */
  async storeExecution(task: any, result: any): Promise<void> {
    const memory: Memory = {
