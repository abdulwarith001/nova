...
export class SkillLoader {
  ...
  getIndexSummary(): string {
    ...
    return [
      ...
      "",
      "=== SCHEDULING RULES ===",
      "CRITICAL: When the user mentions ANY time-related action, USE THE SCHEDULING TOOLS IMMEDIATELY. Do NOT do the task yourself — schedule it.",
      "",
      "TRIGGER PHRASES → ACTIONS:",
      "• 'remind me...' → schedule_create(kind: 'reminder', delayMs: ...)",
      "• 'every day/hour/X min...' → schedule_create(kind: 'recurring', schedule: '24h'/'1h'/'Xm', delayMs: ...)",
      "• 'in X minutes/hours, do Y' → schedule_create(kind: 'task', action: 'Y', delayMs: X*60000). NEVER do Y immediately — always schedule it.",
      "• 'in the next X mins, do Y' → SAME: schedule_create(kind: 'task'). This means 'after X minutes', NOT 'within the next X minutes'.",
      "",
      "PARAMETERS:",
      "• delayMs: milliseconds from now. 1 min = 60000, 1 hour = 3600000, 1 day = 86400000. NEVER pass Unix timestamps.",
      "• If the user does NOT specify a time, ASK once. Never guess.",
      "",
      "MODIFYING/DELETING:",
      "• 'update/change that to X', 'make it every X' → call schedule_list FIRST to get IDs, then schedule_update. NEVER ask which tool to use — just do it.",
      "• When updating a task, update BOTH message AND action together so they stay in sync.",
      "• To cancel or delete: call schedule_list FIRST, then schedule_cancel with the correct ID.",
      "• NEVER create a duplicate — use schedule_update to modify existing items.",
      "",
      "BE DECISIVE:",
